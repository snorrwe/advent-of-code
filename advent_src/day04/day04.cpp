...
using TInput = std::array<char const*, 512>;
using TTest = std::array<char const*, 3>;
using TWords = std::array<char const*, 20>;

constexpr auto TEST = TTest{"aa bb cc dd ee", "aa bb cc dd aa", "aa bb cc dd aaa"};

constexpr auto INPUT =
    TInput{"bdwdjjo avricm cjbmj ran lmfsom ivsof",
...
constexpr auto findWords = [](char const* line) {
    TWords result{};
    result[0] = line;
    size_t size = 1;
    while (*line != '\n' && *line != '\0')
...
constexpr auto hasDupe = [](auto const& words, auto areSame) {
    for (int i = 0; i < words.second; ++i)
    {
        for (int j = 0; j < words.second; ++j)
        {
            if (i != j && areSame(words.first[i], words.first[j]))
            {
                return true;
            }
        }
...
constexpr auto solve = [](auto const& input, auto areSame) {
    size_t result = 0;
    for (int i = 0; i < input.size(); ++i)
    {
        auto words = findWords(input[i]);
        if (!hasDupe(words, areSame))
        {
            result++;
        }
    }
...
constexpr auto areSame_1 = [](char const* lhs, char const* rhs) {
    while ((*lhs != '\0' && *lhs != ' ' && *lhs != '\n') &&
           (*rhs != '\0' && *rhs != ' ' && *rhs != '\n'))
    {
...
{
    constexpr auto result1 = solve(INPUT, areSame_1);
