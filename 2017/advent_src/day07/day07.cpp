...
struct Node
{
    const static size_t CHILDREN_MAX_SIZE = 20;

    Word name;
    size_t childrenSize = 0;
    std::array<Word, CHILDREN_MAX_SIZE> children;
};

const size_t TEST_SIZE = 13;
using TTestData = std::array<char const*, TEST_SIZE>;
using TTestWordCollection = std::array<Node, TEST_SIZE>;

constexpr auto TEST = TTestData{"pbga (66)",
                                "xhth (57)",
                                "ebii (61)",
                                "havc (66)",
                                "ktlj (57)",
...
const size_t ACTUAL_SIZE = 1059;
using TActualData = std::array<char const*, ACTUAL_SIZE>;
using TActualWordCollection = std::array<Node, ACTUAL_SIZE>;

constexpr auto ACTUAL =
    TActualData{"czlmv (78)",
                "fiwbd (57)",
                "twxnswy (98)",
                "dhrnu (62)",
                "yptrqp (64)",
...
constexpr auto ignore = [](char const letter) {
    switch (letter)
    {
    case '1':
    case '2':
...
template <typename T>
constexpr auto tokenize = [](auto const& input, auto size) {
    auto result = T();
    for (int i = 0; i < size; ++i)
    {
        auto line = input[i];
        auto tokens = std::array<Word, 100>();
        size_t tokensSize = 0;
        ...
        {
            ++j;
            if (!inWord && !ignore(line[j]))
            {
                begin = &line[j];
                inWord = true;
            }
            else if (inWord && ignore(line[j]))
            {
                tokens[tokensSize++] = Word(begin, &line[j]);
                inWord = false;
            }
        }
        auto node = Node();
        ...
        {
            node.children[node.childrenSize++] = tokens[i];
        }
        result[i] = node;
    }
    return result;
...
template <typename T>
constexpr auto solve = [](auto const& input, auto size) {
    auto tokens = tokenize<T>(input, size);
    Word result;
    for (auto& node : tokens)
    {
        bool isChildOfAny = false;
        for (int i = 0; i < size && !isChildOfAny; ++i)
        {
            for (int j = 0; j < tokens[i].childrenSize; ++j)
            {
                auto& child = tokens[i].children[j];
                if (child == node.name)
                {
                    isChildOfAny = true;
                }
